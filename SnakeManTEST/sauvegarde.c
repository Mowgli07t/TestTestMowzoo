...
int sauvegarder_partie(int slot, Snake *s, Pomme *p, int score) {
    ...
    if (f != NULL) {
        ...
        /* 2. On écrit la Pomme (x y) */
        fprintf(f, "%d %d\n", p->pos.x, p->pos.y);
        
        /* 3. On écrit la Direction du serpent (dx dy) */
        fprintf(f, "%d %d\n", s->dx, s->dy);

        /* 4. On écrit la Taille puis tout le Corps */
        fprintf(f, "%d\n", s->taille);
...
int charger_partie(int slot, Snake *s, Pomme *p, int *score) {
    ...
    if (f != NULL) {
        ...
        /* Sécurité anti-crash si le fichier est corrompu */
        if(s->taille > SNAKE_MAX_LEN) s->taille = SNAKE_MAX_LEN;
...
int selectionner_slot(float ratio_x, float ratio_y) {
    ...
    while (quitter == 0) {
        ...
        /* Clic sur les 4 carrés (Coordonnées reprises de ton code) */
        ...
        /* Zone invisible au centre pour Annuler/Retourner au jeu (optionnel) */
